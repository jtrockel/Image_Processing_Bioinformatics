...
#TODO make this work for only one image as well 
...
def ela(fname, orig_dir, save_dir, quality = 90):
    """
...
def main ():
    ...
    for d in os.listdir(dirc):
        if d.endswith(".jpg") or d.endswith(".jpeg"):
            thread = threading.Thread(target=ela, args=[d, dirc, ela_dirc, quality])
